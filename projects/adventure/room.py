...
class Room:
    ...
    def print_room_description(self, player): # returns location of player
        print(str(self))
    def get_exits(self): # returns list of exits
        ...
        if self.e_to is not None:
            exits.append("e")
        return exits
    def get_exits_string(self): # returns as string
        return f"Exits: [{', '.join(self.get_exits())}]"
    def connect_rooms(self, direction, connecting_room): #connects direction to room 
        if direction == "n":
            ...
            return None
    def get_room_in_direction(self, direction): # returns room
        if direction == "n":
            return self.n_to
        elif direction == "s":
            return self.s_to
            ...
            return None
    def get_coords(self): # where are you? 
